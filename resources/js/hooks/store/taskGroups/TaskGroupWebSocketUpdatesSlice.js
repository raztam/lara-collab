...
const createTaskWebSocketUpdatesSlice = (set, get) => ({
  addTaskGroupLocally: taskGroup => {
    return set(
      produce(state => {
        // Guard: only add if the group belongs to the current board
        const currentBoardId = route().params?.board;
        if (currentBoardId && taskGroup.board_id && taskGroup.board_id !== currentBoardId) return;
  ...
  restoreTaskGroupLocally: taskGroup => {
    return set(
      produce(state => {
        state.groups = [taskGroup, ...state.groups.filter(i => i.id !== taskGroup.id)].sort(
          (a, b) => (a.order_column > b.order_column ? 1 : -1)
        );
      })
    );
  },
